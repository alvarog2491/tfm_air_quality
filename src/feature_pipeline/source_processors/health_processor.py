...
class HealthProcessor(BaseProcessor):
    ...
    def is_loaded(self) -> bool:
        ...
        return (
            self._respiratory_diseases_df is not None
            and not self._respiratory_diseases_df.empty
            and self._life_expectancy_df is not None
            and not self._life_expectancy_df.empty
        )
    ...
    def load_csv_files(self) -> None:
        ...
        try:
            ...
            self._respiratory_diseases_df = pd.read_csv(
                respiratory_file, parse_dates=["Periodo"], decimal=",", sep=";"
            )
            ...
            self._validate_dataframe_not_empty(
                self._respiratory_diseases_df, respiratory_file
            )
            self._validate_dataframe_not_empty(
                self._life_expectancy_df, life_expectancy_file
            )

            # Log data info
            self._log_dataframe_info(
                self._respiratory_diseases_df, "respiratory diseases"
            )
    ...
    def clean_dataframes(self) -> None:
        ...
        for df_name, df in [
            ("respiratory_diseases", self._respiratory_diseases_df),
            ("life_expectancy", self._life_expectancy_df),
        ]:

            if "Provincias" in df.columns:
                df["Provincias"] = df["Provincias"].str.replace(
                    r"[0-9\s]+", "", regex=True
                )
                df.rename(columns={"Provincias": "Province"}, inplace=True)
                self.logger.info(
                    f"Removed numeric codes on province names in {df_name} dataset"
                )
    ...
    def merge_dataframes(self) -> None:
        ...
        try:
            ...
            self._health_df = self._health_df.drop(
                ["Sexo_respiratory", "Sexo_life_exp", "Causa de muerte"], axis=1
            )
    ...
    def map_province_names(self) -> None:
        ...
        """
        self._respiratory_diseases_df = ProvinceMapper.map_province_name(
            "respiratory_diseases", self._respiratory_diseases_df
        )
        self._life_expectancy_df = ProvinceMapper.map_province_name(
            "life_expectancy", self._life_expectancy_df
        )
