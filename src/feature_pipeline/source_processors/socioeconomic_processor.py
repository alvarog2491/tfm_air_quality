...
class SocioeconomicProcessor(BaseProcessor):
    ...
    def is_loaded(self) -> bool:
        ...
        return (
            self._gdp_df is not None
            and not self._gdp_df.empty
            and self._province_population_df is not None
            and not self._province_population_df.empty
        )
    ...
    def load_csv_files(self) -> None:
        ...
        try:
            ...
            self._gdp_df = pd.read_csv(
                pib_file, sep=";", decimal=",", encoding="ISO-8859-1"
            )
            ...
            self._validate_dataframe_not_empty(
                self._province_population_df, population_file
            )

            # Log data info
            self._log_dataframe_info(self._gdp_df, "GDP")
            self._log_dataframe_info(
                self._province_population_df, "province population"
            )
    ...
    def _transform_population_columns(self) -> None:
        ...
        self._province_population_df.rename(
            columns={"Total": "Population", "Periodo": "Year"}, inplace=True
        )
        self._province_population_df["Population"] = (
            self._province_population_df["Population"]
        ...
        if "Provincias" in self._province_population_df.columns:
            self._province_population_df["Provincias"] = self._province_population_df[
                "Provincias"
            ].str.replace(r"[0-9\s]+", "", regex=True)
            self._province_population_df.rename(
                columns={"Provincias": "Province"}, inplace=True
            )
    ...
    def map_province_names(self) -> None:
        ...
        self._province_population_df = ProvinceMapper.map_province_name(
            "province_population", self._province_population_df
        )
    ...
    def save_processed_file(self) -> None:
        ...
        self._save_dataframe_to_csv(
            self._province_population_df, "province_population_size.csv"
        )
