...
class DataMerger:
    ...
    def merge_all_data(
        ...
        gdp_df: pd.DataFrame,
        province_population_df: pd.DataFrame,
    ) -> pd.DataFrame:
        ...
        try:
            ...
            self._validate_merge_columns(
                airquality_df, health_df, gdp_df, province_population_df
            )
    ...
    def _validate_merge_columns(
        ...
        missing_aq_cols = [
            col for col in required_aq_cols if col not in airquality_df.columns
        ]
        if missing_aq_cols:
            raise ValueError(
                f"Air quality DataFrame missing columns: {missing_aq_cols}"
            )

        # Check health columns
        required_health_cols = ["Province", "Periodo"]
        missing_health_cols = [
            col for col in required_health_cols if col not in health_df.columns
        ]
        ...
        missing_gdp_cols = [
            col for col in required_gdp_cols if col not in gdp_df.columns
        ]
        if missing_gdp_cols:
        ...
        missing_province_pop_cols = [
            col
            for col in required_province_population_cols
            if col not in province_population_df.columns
        ]
        if missing_province_pop_cols:
            raise ValueError(
                f"Province population size DataFrame missing columns: {missing_province_pop_cols}"
            )
