...
def validate_model_config(config: dict) -> None:
    ...
    if missing_keys:
        raise ValidationError(f"Model config missing required keys: {sorted(missing_keys)}")
...
def validate_preprocessing_config(config: dict) -> None:
    """
    ...
    Raises:
        ValidationError: If configuration is invalid
    """
    required_keys = ["categorical_features", "numerical_features", "target_column", "validation_size"]
    missing_keys = set(required_keys) - set(config.keys())
    if missing_keys:
        raise ValidationError(f"Preprocessing config missing required keys: {sorted(missing_keys)}")
